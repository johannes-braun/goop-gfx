...
namespace goop
{
  ...
  void join_into(triangulated_object_t& dst, triangulated_object_t const& src) {
    size_t const base_point = dst.positions.size();
    size_t const base_index = dst.indices.size();

    dst.indices.insert(dst.indices.end(), begin(src.indices), end(src.indices));
    dst.positions.insert(dst.positions.end(), begin(src.positions), end(src.positions));
    dst.normals.insert(dst.normals.end(), begin(src.normals), end(src.normals));
    dst.texcoords.insert(dst.texcoords.end(), begin(src.texcoords), end(src.texcoords));

    for (size_t i = base_index; i != dst.indices.size(); ++i) {
      dst.indices[i] += base_point;
    }
  }
}